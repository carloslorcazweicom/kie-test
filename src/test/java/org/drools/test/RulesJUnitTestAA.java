...
public class RulesJUnitTestAA extends TestCase {

	private static final int THREADS = 5;
	
	private static final int PROCESS = 2;
	
	public RulesJUnitTestAA() {
	}
	...
	public void testConcurrent() {
		...
		for (int i = 0; i < THREADS;i++) {
			ThreadAA t = new ThreadAA(i,PROCESS);
			t.start();
	...
	private class ThreadAA extends Thread {

		private Integer i_;
		
		private Integer totalCalls;

		public ThreadAA(Integer i_, Integer totalCalls) {
			super();
			this.i_ = i_;
		...
		public void run() {
			for (int i = 0;i < totalCalls;i++) {
				...
				paramAB.put("message", new Message("AA" + i_ + "_" + i, new Date()));
				kSession.startProcess("droolsTest.RFGAA", paramAB);
			}
		}
	}
